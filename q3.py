...
def d_lin_reg_th(x, th, th0):
    ...
    """
    #Your code here [1]
    # d/dth (th.T @ x + th0) = x for each example (column of x)
    grad_wrt_th = x
    return grad_wrt_th
...
def d_square_loss_th(x, y, th, th0):
    ...
    """
    #Your code here [2]
    # Using chain rule: d/dth (y_hat - y)^2 = 2*(y_hat - y)*d(y_hat)/dth
    err = lin_reg(x, th, th0) - y
    dlin_dth = d_lin_reg_th(x, th, th0)
    return 2 * err * dlin_dth
...
def d_mean_square_loss_th(x, y, th, th0):
    ...
    """
    #Your code here [3]
    grads = d_square_loss_th(x, y, th, th0)
    mean_grad = np.mean(grads,axis=1, keepdims = True)
    return mean_grad
...
def d_mean_square_loss_th0(x, y, th, th0):
    ...
    """
    #Your code here [6]
    grads = d_square_loss_th0(x, y, th, th0)
    mean_grad = np.mean(grads, axis = 1, keepdims = True)
    return mean_grad
...
def d_ridge_obj_th(x, y, th, th0, lam):
    ...
    """
    #Your code here [7]
    data_term = d_mean_square_loss_th(x, y, th, th0)
    reg_term = 2 * lam * th
...
def d_ridge_obj_th0(x, y, th, th0, lam):
    ...
    """
    #Your code here [8]
    grad_th0 = d_mean_square_loss_th0(x, y, th, th0)
    return grad_th0
...
def sgd(X, y, J, dJ, w0, step_size_fn, max_iter):
    ...
    for i in range(max_iter):
        example_index = i % n
        Xi = X[:, example_index:example_index+1]
        yi = y[:, example_index:example_index+1]
        grad = dJ(Xi, yi, w)
        step_size = step_size_fn(i)
        w = w - step_size * grad
        current_loss = J(Xi, yi, w)
...
def sgdTest():
    ...
    #Your code here [10]
    Xe = np.vstack([X, np.ones((1, X.shape[1]))])
    w0 = np.zeros((Xe.shape[0], 1))
    def step_size(i):
        return 0.1 / (i + 1) ** 0.5
    w, fs, ws = sgd(Xe, y, J, dJ, w0, step_size, 200)
    return w, fs
